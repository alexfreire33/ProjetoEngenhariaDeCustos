...
function populaListagem() {
    ...
    ajaxSubmitRetornoPost("Listar", parametrosListarprincipal, function (returnValue) {
        ...
        try {
            ...
            for (var a = 0; a < jsonListaPadrao.itensDeLevantamentos.length; a++) {


                jQuery("#listagemItem tbody").append("\
                    <tr>\
                        <td><font>" + jsonListaPadrao.itensDeLevantamentos[a]['CodItensLevantamento'] + "</font></td>\
                        <td><font>" + jsonListaPadrao.itensDeLevantamentos[a]['NomeItensLevantamento'] + "</font></td>\
                        <td class='center'>\
                            <input type='hidden' class='cod_item' value='" + JSON.stringify(jsonListaPadrao.itensDeLevantamentos[a]) + "'>\
                            <a href='javascript:void(0)' class='item'><i class='fa fa-pencil'></i><a/>\
                        </td>\
                        <td class='center'>\
                            <input type='hidden' class='jsonParaDeletar' value='" + JSON.stringify(jsonListaPadrao.itensDeLevantamentos[a]) + "'>\
        ...
        ");
            ...
            var options = $("#itensDeLevantamento\\.TbGrupoInsumoCodGrupoInsumo");//essas \\ é para não da problema porque o id tem . no nome

            for (var it = 0; it < jsonListaPadrao.grupoDeInsumos.length; it++) {

                options.append($("<option />").val(jsonListaPadrao.grupoDeInsumos[it].CodGrupoInsumo).text(jsonListaPadrao.grupoDeInsumos[it].NomeGrupoInsumo));
            ...
            var options2 = $("#itensDeLevantamento\\.TbUnidadeMedidaCodUnidadeMedida");//essas \\ é para não da problema porque o id tem . no nome
...
jQuery("body").on("click", ".item", function () {
    ...
    $('#itensDeLevantamento\\.TbGrupoInsumoCodGrupoInsumo').val(itemFiltrado.TbGrupoInsumoCodGrupoInsumo).change();
    $('#itensDeLevantamento\\.TbUnidadeMedidaCodUnidadeMedida').val(itemFiltrado.TbUnidadeMedidaCodUnidadeMedida).change();
    ...
    jQuery("#itensDeLevantamento_CodItensLevantamento").val(itemFiltrado.CodItensLevantamento);//coloca no hidden o codigo a ser editado
...
jQuery("body").on("click", ".valores_deletar", function () {
    ...
    swal({
        ...
        function () {
            ...
            jQuery("#itensDeLevantamento_CodItensLevantamento").val(itemFiltradoParaDelete.CodItensLevantamento);//coloca no hidden o codigo a ser editado
